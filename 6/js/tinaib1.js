var postModule = function () {
    ...
    var parseThread = function (html) {
        ...
        [].forEach.call(refs, function (link) {
            ...
            var rNum = link.href.match(/\d+$/); // куда ссылается
            if (!rNum) {
                return;
            }
...
// up and down buttons
document.addEventListener('DOMContentLoaded', function () {
    if (mobile && !/Opera|OPR\/\d/.test(navigator.userAgent)) {
...
window.addEventListener('load', function () {
    ...
    addScript('/js/pushstream.js', function () {
        var pieces = window.location.hostname.split('.');
        var tld = pieces.pop();
        var myhost = 'comet.' + pieces.pop() + '.' + tld;
