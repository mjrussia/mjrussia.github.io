...
function aj_get_binary1(url, fn, o) {
    var xhr = new XMLHttpRequest();
    if (o && o.binary && xhr.overrideMimeType) {
...
function aj_get_binary2(url, fn, o) {
...
function doArchiveThread() {
    ...
    var msgWindow = window.open('', "msgWindow", "width=520,height=500,left=100,top=100,menubar=no,directories=no,location=no,scrollbars=yes,resizable=yes,status=no");
    ...
    if (!msgWindow.document.body) {
        msgWindow.document.write('<html><head><title>Сохранение треда</title></head><body><p>Please, wait..</p></body></html>');
    ...
    var logItem = function() {
        ...
        return {
            ...
            unrotate: function() {
                if (rot_interval)
                    window.clearInterval(rot_interval);
    ...
    $each($X('.//img[@class="thumb"]'), function(th) { // fetch files and thumbs
        var id = th.getAttribute("id").substr(5); // post id
        pending += 1;
        var th_logger = logItem();
        th_logger.say("Скачивание превью №" + id).rotate();
        aj_get_binary2(th.src, function(str) {
            th_logger.unrotate().say("Скачивание превью №" + id + " [OK]");
            files.push({
                'path': "thumb/" + th.src.replace(/.*\/thumb\/(.*)$/, "$1"),
                'content': str
            });
            // zip.file("thumb/" + th.src.replace(/.*\/thumb\/(.*)$/, "$1"), str, {  binary: true });
            pending -= 1;
            zip_pending += 1;
        }, {
            binary: true
        ...
        pending += 1;
        var f_logger = logItem();
        f_logger.say("Скачивание файла №" + id).rotate();
        aj_get_binary2(th.parentNode.href, function(str) {
            f_logger.unrotate().say("Скачивание файла №" + id + " [OK]");
            files.push({
                'path': "src/" + th.parentNode.href.replace(/.*\/src\/(.*)$/, "$1"),
                'content': str
            });
            // zip.file("src/" + th.parentNode.href.replace(/.*\/src\/(.*)$/, "$1"), str, { binary: true  });
            pending -= 1;
            zip_pending += 1;
        }, {
            binary: true
    ...
    $each($X(".//script[@src]"), function(script) { // fetch scripts
        if (script.src.indexOf("jszip") != -1)
            return;

        pending += 1;
        var sc_logger = logItem();
        sc_logger.say("Скачивание скрипта " + script.src).rotate();
        aj_get_binary1(script.src, function(str) {
            sc_logger.unrotate().say("Скачивание скрипта " + script.src + " [OK]");
            zip.file("js/" + script.src.replace(/.*\/js\/(.*)$/, "$1"), str);
            pending -= 1;
        });
    });

    $each($X('.//head/link[@rel="stylesheet"]'), function(style) { // fetch styles
        pending += 1;
        var st_logger = logItem();
        st_logger.say("Скачивания стиля " + style.href).rotate();
        aj_get_binary1(style.href, function(str) {
            st_logger.unrotate().say("Скачивания стиля " + style.href + " [OK]");
            zip.file("css/" + style.href.replace(/.*\/css\/(.*)$/, "$1"), str);
            pending -= 1;
        });
    });
    ...
    var reduce = function() {
        ...
        var zlogger = logItem();
        zlogger.say("Добавление в архив файла " + zfile.path).rotate();
        zip.file(zfile.path, zfile.content, {
            binary: true
        });
        zlogger.unrotate().say("Добавление в архив файла " + zfile.path + " [OK]");
        zip_pending -= 1;
        return setTimeout(reduce, 300);
    };
