        ...
        $get("/" + brd + "/res/" + tNum + ".html", function (response) {
            ...
            for (var i = 0; i < ajaxPosts[tNum].keys.length; i += 1) {
                ...
                if (!postByNum[pNum] && pNum != tNum) {
                    ...
                    dummy.innerHTML = '<table><tbody><tr><td class="doubledash"> >> </td><td class="reply" id="reply' + pNum + '">' //
                            + ajaxPosts[tNum][pNum] + '</td></tr></tbody></table>';
                    var cln = dummy.firstChild;
...
// pnum contains >>rnum
    ...
    var doRefMap = function () {
        ...
        var refs = [].filter.call(document.forms['delform'].getElementsByTagName('a'),
                function (el) {
                    var text = el.textContent.replace(/[\u00AD\u002D\u2011]+/g, "");
                    return (/^\>\>(?:\/?\S{1,4}\/)?\d+$/.test(text)
                            && !/^rfmap/.test(el.parentNode.parentNode.id));
                });
        ...
        [].forEach.call(refs, function (link) {
            ...
            if (post) {
                getRefMap(post.id.match(/\d+/)[0], rNum);
            }
        });
...
function doPostForm() {
    ...
    var noko = form.noko;
    if (noko && /i\d+/.test(document.location.hash) == false) {
        noko.checked = true;
    }

    // draft key
    var key = document.location.pathname.match(/\/res\/(\d+)\.html$/i) ?
            document.location.pathname.match(/\/res\/(\d+)\.html$/i)[1] : "index";
    ...
    iframe.onload = function () {
        ...
        if (h1 && (h1.innerHTML.indexOf(txt[lang]['js_error']) != -1)) { // error
            ...
            if (window.localStorage) {
                window.localStorage.removeItem("draft_" + brd + "_" + key);
            }
    ...
    var css_challenge = function () {
        ...
        dummy.innerHTML = '<iframe class="ninja" id="csstest" src="about:blank"></iframe>';
    ...
    if (window.localStorage) {
        ...
        //add del button
        var deldraft = document.createElement('span');
        deldraft.className = 'filesize';
        deldraft.style.verticalAlign = 'top';
        deldraft.style.margin = '0 .2em';
        deldraft.style.padding = '.2em .3em';
        deldraft.style.cursor = 'pointer';
        deldraft.style.visibility = 'hidden';
        deldraft.style.position = "relative";
        deldraft.style.right = "1.4em";
        deldraft.innerHTML = 'x';
        ...
        insertAfter(form.message, deldraft);
        // load draft
        var draft = window.localStorage.getItem("draft_" + brd + "_" + key);
        if (draft) {
            ...
            deldraft.style.visibility = 'visible';
        }
        var mesg0;

        var save_draft = function () {
            try {
                localStorage.setItem("draft_" + brd + "_" + key, form.message.value);
        ...
        var draftint = setInterval(function () {
            if (form.message.value && mesg0 != form.message.value // changed
                    && window.localStorage.getItem("draft_" + brd + "_" + key) == mesg0 //saved on this page before
                    ) {
                save_draft();
                deldraft.style.visibility = 'visible';
            }
...
function doStars(root) {
    ...
    [].forEach.call(
            (root || document.forms['delform']).getElementsByClassName('postertrip vip')
            , function (block) {
    ...
    [].forEach.call(document.links, function (a) {
        ...
        if (m && /eply|твет/.test(a.innerHTML)) {
            if (window.localStorage && window.localStorage.getItem("draft_" + brd + "_" + m[1])) {
                var dummy = document.createElement('p');
                dummy.innerHTML = '<span class="thumbnailmsg">(' + txt[lang]['saved_draft'] + ')</span>';
                insertAfter(a.nextSibling, dummy.firstChild);
            }
        }
    });
}
