...
function doExpand(node) {
    Array.prototype.forEach.call((node || document).getElementsByClassName('thumb'),
...
function doPostForm() {
        ...
        iframe.onload = function() {
            ...
            if (h1 && (h1.innerHTML.indexOf(txt[lang]['js_error']) != -1)) {
                ...
                if (window.localStorage) {
                    window.localStorage.removeItem("draft_" + brd + "_" + res);
                }
    ...
    var css_challenge = function() {
        ...
        dummy.innerHTML = '<iframe class="ninja" id="csstest" src="/' + brd + '/csstest.foo"></iframe>';
    ...
    if (window.localStorage) {
        //add del button
        var dummy = document.createElement('p');
        dummy.innerHTML = '<span class="shortened"> x </span>';
        var deldraft = dummy.firstChild;
        ...
        // load draft
        var draft = window.localStorage.getItem("draft_" + brd + "_" + res);
        if (draft) {
        ...
        var save_draft = function() {
            ...
            window.localStorage.setItem("draft_" + brd + "_" + res, form.message.value);
        };
        // save loop 
        ...
        window.draftint = setInterval(function() {
            if (form.message.value && mesg0 != form.message.value) {
                save_draft();
                deldraft.style.display = '';
            }
        }, 10000);
    ...
    var fn = function(a) {
        ...
        if (m && /eply|твет/.test(a.innerHTML)) {
            if (window.localStorage && window.localStorage.getItem("draft_" + brd + "_" + m[1])) {
                var dummy = document.createElement('p');
                dummy.innerHTML = '<span class="thumbnailmsg">(есть черновик)</span>';
